        ...
        if i != data_ind:
            ...
            if(dist < thresh_dist):
                ...
                if counter > thresh_count:
                    break
    return counter
def remove_noise(data,thresh_dist, thresh_count = 50):
...
def runA1(): 
    ...
    res_arr = remove_noise(geom_arr, thresh_dist)
    print(' ')
    print(res_arr.shape)
    scr.convert_np_ply(geom_arr,col_arr,'calib0.ply', overwrite = True)
    col_arr1 = scr.gen_color_arr_black(res_arr.shape[0])
    scr.convert_np_ply(res_arr,col_arr1,'calib1.ply', overwrite = True)
    print(find_centroid(geom_arr))
runA1()