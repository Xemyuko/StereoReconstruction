...
def load_mats(folder, kL_file = "kL.txt", 
    ...
    return kL, kR, r, t
def create_data_out(ptsL, ptsR, unrLX, unrRX,unrLY, unrRY, cor, geom, col, filename1):
    '''
    ...
    with open(file_check, 'w') as ori:  
        ori.write("'x1', 'y1', 'x2', 'y2', 'unrLX','unrRX','unrLY','unrRY','corr', 'x', 'y', 'z', 'r', 'g', 'b'\n")
        for i in range(len(ptsL)):
            ori.write(str(ptsL[i][0]) + " " + str(ptsL[i][1]) + " " + str(ptsR[i][0]) + " " + str(ptsR[i][1])+
                      " " + str(unrLX[i]) + " " + str(unrRX[i]) + " " + str(unrLY[i]) + " " + str(unrRY[i]) +
...
def cal_comp(images, ext, rows, columns, world_scaling, objp):
...
def calibrate_single(images, ext, rows, columns, world_scaling):
    ...
    cont_fac = 1
    imgpoints, objpoints = cal_comp(images, ext, rows, columns, world_scaling, objp)
    if len(imgpoints) == 0:
        print('Corners not found.')
        print('Retrying with contrast increase...')
        img2 = []
        for i in images:
            cont_fac = 2
            frame = boost_zone(i,cont_fac)
            img2.append(frame)
        imgpoints, objpoints = cal_comp(img2, ext, rows, columns, world_scaling, objp)
        if len(imgpoints) == 0:
            print('Corners not found.')
            print('Retrying with additional contrast increase...')
            img3 = []
            cont_fac = 5
            for i in images:
                frame = boost_zone(i,cont_fac)
                img3.append(frame)
            imgpoints, objpoints = cal_comp(img3, ext, rows, columns, world_scaling, objp)
    if(len(imgpoints) > 0):        
...
def calibrate_cameras(cal_folder, left_mark, right_mark, ext, rows, columns, world_scaling):
    ...
    mtx1, dist_1, fac1 = calibrate_single(images1, ext, rows, columns, world_scaling)
    if mtx1 is not None:
        print("Calibrating Right Camera.")
        mtx2, dist_2, fac2 = calibrate_single(images2, ext, rows, columns, world_scaling)
        if mtx2 is not None:
