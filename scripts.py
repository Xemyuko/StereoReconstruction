...
def build_image_list( ext,folder = ""):
    '''
    Creates alphabetically sorted list of files with a given extension in a folder. 
    Defaults to the folder the code is in.
    ...
    folder : string, optional
        folder to access, ending in /. The default is "".
...
def load_images(folder = "", ext = ".pgm"):
    '''
    Loads images that have the file name in a format similar to:
    "Kamera-1-POS010_Aufnahme000.pgm" 
    from a folder
    Uses the value of the first '1' to determine if the image is from the 
    left or right camera
    Parameters
    ----------
    folder : string, optional
        folder location. The default is "".
    ext : string, optional
        extension to be used. The default is ".pgm".

    Returns
    -------
    imgL : list
        left camera images list
    imgR : list
        right camera images list

    '''
    image_list = build_image_list(ext, folder)
    imgL = []
    imgR = []
    for i in image_list:
...
def load_img_batch(folder, ext):
    '''
    Loads all images in folder with extension provided in alphabetical order
